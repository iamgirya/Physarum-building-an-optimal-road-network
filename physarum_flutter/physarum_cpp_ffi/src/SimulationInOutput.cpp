// Использовался для чтения настроек через файл и вывод в консоль. Сейчас более не нужен
...
//	vector<pair<it, it>> points;
//	for (int i = 0; i < particles.size(); i++) {
//		points.push_back(make_pair(particles[i]->pixelVector[0], particles[i]->pixelVector[1]));
//	}
//	vector<vector<bool>> tmp = vector<vector<bool>>(location.trailMap.size(), vector<bool>(location.trailMap[0].size(), false));
//	for (int i = 0; i < points.size(); i++) {
//		tmp[points[i].first][points[i].second] = true;
//	}
//
//	for (int i = 0; i < location.trailMap.size(); i++) {
//		for (int j = 0; j < location.trailMap[0].size(); j++) {
...
//			for (int i = 0; i < points.size(); i++) {
//				if (img[(points[i].first + points[i].second * w) * 3 + colorOffset] > (255 - 8) * colorVector[colorOffset]) {
//					img[(points[i].first + points[i].second * w) * 3 + colorOffset] += 8 * colorVector[colorOffset];
//				}
//				else {
//					img[(points[i].first + points[i].second * w) * 3 + colorOffset] = 255 * colorVector[colorOffset];
//				}
//			}
//		}
//	}
//	else {
//		for (int i = 0; i < points.size(); i++) {
//			if (img[(points[i].first + points[i].second * w) * 3 + 2] > 255 - 8) {
//				img[(points[i].first + points[i].second * w) * 3 + 2] += 8;
//			}
//			else {
...
//}
//
//void SlimeMoldSimulation::outputInBmpGraph(vector<pair<it,it>>& points, vector<vector<it>>& graph, bool isChangedSettings = false, int number = 0) {
//	string num = "1";
//	if (isChangedSettings) {
